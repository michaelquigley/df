...
func TestMatchConstraint(t *testing.T) {
	t.Run("successful string match", func(t *testing.T) {
		type Config struct {
			Version string `dd:"version,+match=\"1.0.0\""`
			Name    string `dd:"name"`
		}
	...
	t.Run("successful boolean match", func(t *testing.T) {
		type Config struct {
			Debug bool `dd:"debug,+match=\"true\""`
		}
	...
	t.Run("match with custom field name", func(t *testing.T) {
		type Config struct {
			APIVersion string `dd:"api_version,+match=\"v1\""`
		}
	...
	t.Run("match with required flag", func(t *testing.T) {
		type Config struct {
			Mode string `dd:"mode,+required,+match=\"production\""`
		}
	...
	t.Run("match with secret flag", func(t *testing.T) {
		type Config struct {
			KeyType string `dd:"key_type,+secret,+match=\"rsa\""`
		}
	...
	t.Run("missing required field with match constraint", func(t *testing.T) {
		type Config struct {
			Version string `dd:"version,+required,+match=\"1.0.0\""`
		}
	...
	t.Run("nested struct with match constraint", func(t *testing.T) {
		type Database struct {
			Driver string `dd:"driver,+match=\"postgresql\""`
		}

		type Config struct {
			DB Database `dd:"database"`
		}
...
func TestMatchConstraintParsing(t *testing.T) {
	t.Run("basic match parsing", func(t *testing.T) {
		type TestStruct struct {
			Field string `dd:"field,+match=\"value\""`
		}
	...
	t.Run("match with other flags", func(t *testing.T) {
		type TestStruct struct {
			Field string `dd:"custom_name,+required,+secret,+match=\"test\""`
		}
	...
	t.Run("match as first token", func(t *testing.T) {
		type TestStruct struct {
			Field string `dd:"+match=\"first\",+required"`
		}
...
func TestMatchConstraintWithMerge(t *testing.T) {
	...
	t.Run("unquoted match values work in binding", func(t *testing.T) {
		type Config struct {
			QuotedVersion   string `dd:"quoted_version,+match=\"1.0.0\""`
			UnquotedVersion string `dd:"unquoted_version,+match=1.0.0"`
		}
